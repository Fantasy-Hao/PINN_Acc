...
class PhysicsInformedNN():
    ...
    def __init__(self, x, y, t, u, v, p, layers):
        ...
        self.loss_log_name = f'../logs/3_dynamic_{current_time}'
...
if __name__ == "__main__":
    ...
    with open(model.loss_log_name + '_uv.txt', 'w') as file:
        ...
        for i in range(39):
            ...
            idx = np.random.choice(N * T, N_train, replace=False)
            x_train = torch.tensor(x[idx, :], requires_grad=True).float().to(device)
            y_train = torch.tensor(y[idx, :], requires_grad=True).float().to(device)
            t_train = torch.tensor(t[idx, :], requires_grad=True).float().to(device)
            u_train = torch.tensor(u[idx, :]).float().to(device)
            v_train = torch.tensor(v[idx, :]).float().to(device)
            p_train = torch.tensor(p[idx, :]).float().to(device)

            model.x, model.y, model.t = x_train, y_train, t_train
            model.u, model.v, model.p = u_train, v_train, p_train
    ...
    torch.save(model.dnn.state_dict(), '../checkpoints/3_dynamic.pth')
    print("Model saved.")
    ...
    plot_solution(X_star, u_pred, 1, "dynamic_u_pred.png")
    plot_solution(X_star, v_pred, 2, "dynamic_v_pred.png")
    plot_solution(X_star, p_pred, 3, "dynamic_p_pred.png")

    plot_solution(X_star, u_star - u_pred, 4, "dynamic_u_error.png")
    plot_solution(X_star, v_star - v_pred, 5, "dynamic_v_error.png")
    plot_solution(X_star, p_star - p_pred, 6, "dynamic_p_error.png")
