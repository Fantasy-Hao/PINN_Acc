...
class PhysicsInformedNN():
    ...
    def __init__(self, x, y, t, u, v, p, layers):
        ...
        self.loss_log_name = f'../logs/5_RAD_{current_time}'
...
if __name__ == "__main__":
    ...
    X_idx = np.random.choice(N * T, N_train, replace=False)
    ...
    with open(model.loss_log_name + '_uv.txt', 'w') as file:
        ...
        for i in range(39):
            ...
            err_eq = np.power(f_pred, k) / np.power(f_pred, k).mean() + c
            err_eq_normalized = (err_eq / sum(err_eq))[:, 0]  # 归一化误差权重
            X_append_idx = np.random.choice(a=len(idx), size=N_train, replace=False, p=err_eq_normalized)
            ...
            model.x, model.y, model.t = x_append, y_append, t_append
            model.u, model.v, model.p = u_append, v_append, p_append
    ...
    torch.save(model.dnn.state_dict(), '../checkpoints/5_RAD.pth')
    print("Model saved.")
    ...
    plot_solution(X_star, u_pred, 1, "rad_u_pred.png")
    plot_solution(X_star, v_pred, 2, "rad_v_pred.png")
    plot_solution(X_star, p_pred, 3, "rad_p_pred.png")

    plot_solution(X_star, u_star - u_pred, 4, "rad_u_error.png")
    plot_solution(X_star, v_star - v_pred, 5, "rad_v_error.png")
    plot_solution(X_star, p_star - p_pred, 6, "rad_p_error.png")
