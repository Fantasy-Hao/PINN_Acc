...
class PhysicsInformedNN():
    ...
    def __init__(self, x, y, t, u, v, p, layers):
        ...
        self.dnn = modified_MLP(layers).to(device)
        ...
        self.loss_log_name = f'../logs/11_SA+RBA+mMLP_{current_time}'
        ...
        self.lambda_u = self.lambda_v = 0.00

        # Self-Adaptive PINNs
        w_u_size = self.u.detach().cpu().numpy().size
        w_v_size = self.v.detach().cpu().numpy().size
        w_f_u_size = self.u.detach().cpu().numpy().size
        w_f_v_size = self.v.detach().cpu().numpy().size

        self.w_u = torch.rand(w_u_size, 1, requires_grad=True).float().to(device)
        self.w_v = torch.rand(w_v_size, 1, requires_grad=True).float().to(device)
        self.w_f_u = torch.rand(w_f_u_size, 1, requires_grad=True).float().to(device)
        self.w_f_v = torch.rand(w_f_v_size, 1, requires_grad=True).float().to(device)
        self.w_u = torch.nn.Parameter(self.w_u)
        self.w_v = torch.nn.Parameter(self.w_v)
        self.w_f_u = torch.nn.Parameter(self.w_f_u)
        self.w_f_v = torch.nn.Parameter(self.w_f_v)

        self.optimizer_sa = torch.optim.Adam(
            [self.w_u] + [self.w_v] + [self.w_f_u] + [self.w_f_v],
            lr=0.001,
            maximize=True
        )
    ...
    def loss_func(self):
        ...
        self.lambda_u = (self.gamma_u * self.lambda_u + f_u_norm).detach()
        self.lambda_v = (self.gamma_v * self.lambda_v + f_v_norm).detach()

        loss = torch.mean((self.w_u * (u_pred - self.u)) ** 2) + \
               torch.mean((self.w_v * (v_pred - self.v)) ** 2) + \
               torch.mean((self.lambda_u * self.w_f_u * f_u_pred) ** 2) + \
               torch.mean((self.lambda_u * self.w_f_v * f_v_pred) ** 2)
...
if __name__ == "__main__":
    ...
    torch.save(model.dnn.state_dict(), '../checkpoints/11_SA+RBA+mMLP.pth')
    print("Model saved.")
    ...
    plot_solution(X_star, u_pred, 1, "sa+rba+mmlp_u_pred.png")
    plot_solution(X_star, v_pred, 2, "sa+rba+mmlp_v_pred.png")
    plot_solution(X_star, p_pred, 3, "sa+rba+mmlp_p_pred.png")

    plot_solution(X_star, u_star - u_pred, 4, "sa+rba+mmlp_u_error.png")
    plot_solution(X_star, v_star - v_pred, 5, "sa+rba+mmlp_v_error.png")
    plot_solution(X_star, p_star - p_pred, 6, "sa+rba+mmlp_p_error.png")
