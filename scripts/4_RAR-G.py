...
class PhysicsInformedNN():
    ...
    def __init__(self, x, y, t, u, v, p, layers):
        ...
        self.loss_log_name = f'../logs/4_RAR-G_{current_time}'
    ...
    def predict(self, x_star, y_star, t_star):
        ...
        u, v, p, f_u, f_v = self.net_NS(x_star, y_star, t_star)

        u_star = u.detach().cpu().numpy()
        v_star = v.detach().cpu().numpy()
        p_star = p.detach().cpu().numpy()
        f_u_star = f_u.detach().cpu().numpy()
        f_v_star = f_v.detach().cpu().numpy()

        return u_star, v_star, p_star, f_u_star, f_v_star
...
if __name__ == "__main__":
    ...
    # 创建训练集：从原始数据集中随机抽取(1 / 2)样本点（不放回抽样）
    X_idx = np.random.choice(N * T, N_train // 2, replace=False)
    x_train, y_train, t_train = x[X_idx, :], y[X_idx, :], t[X_idx, :]
    u_train, v_train, p_train = u[X_idx, :], v[X_idx, :], p[X_idx, :]
    ...
    with open(model.loss_log_name + '_uv.txt', 'w') as file:
        ...
        for i in range(20):
            u_calc, v_calc, p_calc, _, _ = model.predict(x_test, y_test, t_test)
            ...
            idx = np.random.choice(N * T, 20000, replace=False)
            x_star, y_star, t_star = x[idx, :], y[idx, :], t[idx, :]
            u_star, v_star, p_star = u[idx, :], v[idx, :], p[idx, :]
            u_pred, v_pred, p_pred, f_u_pred, f_v_pred = model.predict(x_star, y_star, t_star)
            f_pred = np.sum((abs(f_u_pred), abs(f_v_pred)), axis=0)

            err_eq = torch.tensor(np.abs(f_pred)[:, 0])
            X_append_idx = torch.topk(err_eq, N_train // 40, dim=0)[1].numpy()

            x_append = torch.tensor(x_star[X_append_idx, :], requires_grad=True).float().to(device)
            y_append = torch.tensor(y_star[X_append_idx, :], requires_grad=True).float().to(device)
            t_append = torch.tensor(t_star[X_append_idx, :], requires_grad=True).float().to(device)
            u_append = torch.tensor(u_star[X_append_idx, :]).float().to(device)
            v_append = torch.tensor(v_star[X_append_idx, :]).float().to(device)
            p_append = torch.tensor(p_star[X_append_idx, :]).float().to(device)

            model.x = torch.cat((model.x, x_append), dim=0)
            model.y = torch.cat((model.y, y_append), dim=0)
            model.t = torch.cat((model.t, t_append), dim=0)
            model.u = torch.cat((model.u, u_append), dim=0)
            model.v = torch.cat((model.v, v_append), dim=0)
            model.p = torch.cat((model.p, p_append), dim=0)
    ...
    torch.save(model.dnn.state_dict(), '../checkpoints/4_RAR-G.pth')
    print("Model saved.")
    ...
    u_pred, v_pred, p_pred, _, _ = model.predict(x_star, y_star, t_star)

    plot_solution(X_star, u_pred, 1, "rarg_u_pred.png")
    plot_solution(X_star, v_pred, 2, "rarg_v_pred.png")
    plot_solution(X_star, p_pred, 3, "rarg_p_pred.png")

    plot_solution(X_star, u_star - u_pred, 4, "rarg_u_error.png")
    plot_solution(X_star, v_star - v_pred, 5, "rarg_v_error.png")
    plot_solution(X_star, p_star - p_pred, 6, "rarg_p_error.png")
